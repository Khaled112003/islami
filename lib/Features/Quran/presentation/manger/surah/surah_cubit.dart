    ...
    if (state is SurahSuccess) {
      ...
      if (query.isEmpty) {
        ...
        final filteredSurahs = currentState.allSurahs.where((surah) {
          final surahNameArabic = removeDiacritics(surah.name?.toLowerCase().trim() ?? '');
          final surahNameEnglish = surah.englishName?.toLowerCase().trim() ?? '';
          
          final queryWithoutDiacritics = removeDiacritics(query.toLowerCase().trim());
